...
QCPLayoutElement *QCPLayoutGrid::takeAt(int index)
{
  ...
  {
    releaseElement(el);
	mElements[index / columnCount()][index % columnCount()] = nullptr;
    return el;
  } else
  {
...
void QCPGraph::setDataValueError(const QVector<double> &key, const QVector<double> &value, const QVector<double> &valueError)
{
  ...
  for (int i=0; i<n; ++i)
  {
	newData.key = key.at(i);
	newData.value = value.at(i);
	newData.valueErrorMinus = valueError.at(i);
	newData.valueErrorPlus = valueError.at(i);
	mData->insertMulti(key.at(i), newData);
  }
}
...
void QCPGraph::setDataValueError(const QVector<double> &key, const QVector<double> &value, const QVector<double> &valueErrorMinus, const QVector<double> &valueErrorPlus)
{
  ...
  for (int i=0; i<n; ++i)
  {
	newData.key = key.at(i);
	newData.value = value.at(i);
	newData.valueErrorMinus = valueErrorMinus.at(i);
	newData.valueErrorPlus = valueErrorPlus.at(i);
	mData->insertMulti(key.at(i), newData);
  }
}
...
void QCPGraph::setDataKeyError(const QVector<double> &key, const QVector<double> &value, const QVector<double> &keyError)
{
  ...
  for (int i=0; i<n; ++i)
  {
	newData.key = key.at(i);
	newData.value = value.at(i);
	newData.keyErrorMinus = keyError.at(i);
	newData.keyErrorPlus = keyError.at(i);
	mData->insertMulti(key.at(i), newData);
  }
}
...
void QCPGraph::setDataKeyError(const QVector<double> &key, const QVector<double> &value, const QVector<double> &keyErrorMinus, const QVector<double> &keyErrorPlus)
{
  ...
  for (int i=0; i<n; ++i)
  {
	newData.key = key.at(i);
	newData.value = value.at(i);
	newData.keyErrorMinus = keyErrorMinus.at(i);
	newData.keyErrorPlus = keyErrorPlus.at(i);
	mData->insertMulti(key.at(i), newData);
  }
}
